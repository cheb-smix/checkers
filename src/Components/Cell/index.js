...
export default class Cell extends React.Component{
    ...
    render(){
        ...
        const {x,y,k,variable,checker,color,active,onCheckerClick} = this.props;
        ...
        if(checker !== false){
            ...
            return (
                <div onClick={()=>onCheckerClick(koords)} koords={koords} className={classNames} x={x} y={y} k={k} variable={p}><Checker key={checker} k={k} color={color} checker={checker} active={active} /><div className="empty" >&nbsp;</div></div>
            );
        }else{
            return (
